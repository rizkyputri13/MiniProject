...
import { EditContractRequest } from "../redux-saga/Action/CandAction";


export default function ModalContract({ dataContract }) {
  let [isOpen, setIsOpen] = useState(false);

  const contractStatus = ["Contracted"];

  const dispatch = useDispatch();

  const formik = useFormik({
    initialValues: {
      boapEntityId: dataContract.boapEntityId,
      boapStatus: dataContract.boapStatus,
    },
    validationSchema: Yup.object().shape({
      boapEntityId: Yup.number().required(),
      boapStatus: Yup.string()
        .oneOf(contractStatus, "Please select status ")
        .required("Please"),
    }),

    onSubmit: async (values) => {
      dispatch(EditContractRequest(values));
      closeModal();
    },
  });
              ...
              >
                <Dialog.Panel className="transform overflow-hidden rounded-md bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="py-6 px-6 lg:px-8">               
                    <div>
                      Kandidat : {dataContract.boapEntity.userFirstName}{" "}
                      {dataContract.boapEntity.userLastName}{" "}
                    </div>
                    <br></br>
                    <div>
                        Set status :{" "}
