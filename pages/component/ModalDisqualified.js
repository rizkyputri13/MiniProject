...
import { EditDisqualifiedRequest } from "../redux-saga/Action/CandAction";

export default function ModalDisqualified({ dataDisq }) {
  let [isOpen, setIsOpen] = useState(false);
  const disqStatus = ["Ready Test", "Not Responding"];

  const dispatch = useDispatch();

  const formik = useFormik({
    initialValues: {
      boapEntityId: dataDisq.boapEntityId,
      boapStatus: dataDisq.boapStatus,
    },
    validationSchema: Yup.object().shape({
      boapEntityId: Yup.number().required(),
      boapStatus: Yup.string()
        .oneOf(disqStatus, "Please select status ")
        .required("Please"),
    }),

    onSubmit: async (values) => {
      dispatch(EditDisqualifiedRequest(values));
      closeModal();
    },
  });
                ...
                <Dialog.Panel className="transform overflow-hidden rounded-md bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <form onSubmit={formik.handleSubmit}>
                    <div className="py-6 px-6 lg:px-8">
                      <div>
                        Kandidat : {dataDisq.boapEntity.userFirstName}{" "}
                        {dataDisq.boapEntity.userLastName}{" "}
                      </div>
                      <br></br>
                      <div>
                        Set status :{" "}
