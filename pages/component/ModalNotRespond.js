...
import { EditNotrespondRequest } from "../redux-saga/Action/CandAction";

export default function ModalApply({ dataNot }) {
  let [isOpen, setIsOpen] = useState(false);
  const notStatus = ["Ready Test", "Not Responding"];

  const dispatch = useDispatch();

  const formik = useFormik({
    initialValues: {
      boapEntityId: dataNot.boapEntityId,
      boapStatus: dataNot.boapStatus,
    },
    validationSchema: Yup.object().shape({
      boapEntityId: Yup.number().required(),
      boapStatus: Yup.string()
        .oneOf(notStatus, "Please select status ")
        .required("Please"),
    }),

    onSubmit: async (values) => {
      dispatch(EditNotrespondRequest(values));
      closeModal();
    },
  });
                ...
                <Dialog.Panel className="transform overflow-hidden rounded-md bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <form onSubmit={formik.handleSubmit}>
                    <div className="py-6 px-6 lg:px-8">
                      <div>
                        Kandidat : {dataNot.boapEntity.userFirstName}{" "}
                        {dataNot.boapEntity.userLastName}{" "}
                      </div>
                      <br></br>
                      <div>
                        Set status :{" "}
